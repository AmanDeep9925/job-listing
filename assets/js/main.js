...
function createJob(item) {
    return `
        ${createBorder(item)}
        <img alt="${item.company}" class="card__profile" src="${item.logo}"/>
        ...
        <div class="card__fields">
            ...
            <span class="card__span__separator"> ${item.contract} </span>
...
function createFlags(data) {
    let outputString = "";
    if (data.new) outputString += '<span class="flag flag__new">New!</span>';
    if (data.featured)
        outputString += '<span class="flag flag__featured">featured</span>';
...
function displayFilter() {
    if (filterContainer.classList.contains("inactive"))
        filterContainer.classList.remove("inactive");
